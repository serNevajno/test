<script>
  function uploadFileAksioma() {
    var formData = new FormData(document.getElementById('askioma'));
    $('#askioma').hide();
    $.ajax({
      url: '/adminius/inc/uploadProduct/uploadAksXML.inc.php',
      data: formData,
      processData: false,
      contentType: false,
      type: 'POST',
  ...
  function uploadProductCSV(countPr, total){
    ...
    $.ajax({
      type: "POST",
      url: "/adminius/inc/uploadProduct/upProductAks.inc.php",
      dataType: "html",
      data: "count="+countPr,
  ...
  <div class="col-md-12">
    <div class="tabbable tabbable-custom boxless">
      ...
      <div class="note note-warning">
        <p>
          Загрузка товаров может занять продолжительное время, не закрывайте страницу до окончание процедуры. Прайс должен быть в формате XML(Таблица XML 2003), а также иметь в первой ячейке индификатор: disk5.
        </p>
      </div>
      <div id="formUp">
        <div class="form-group">
          <h3>Добавление дисков</h3>
          <form method="post" enctype="multipart/form-data" id="askioma">
            <div class="margin-top-10 fileupload fileupload-new" data-provides="fileupload">
              <div class="input-group input-group-fixed">
                ...
                <span class="btn default btn-file">
                                            ...
                                            <input type="file" class="default" name="aks" id="uploadimage"/>
                                        </span>
                <a href="#" class="btn red fileupload-exists" data-dismiss="fileupload"><i class="fa fa-trash-o"></i> Удалить</a>
                <button type="button" class="btn red" onClick="uploadFileAksioma()">Загрузить</button>
              </div>
            </div>
          </form>
        </div>
