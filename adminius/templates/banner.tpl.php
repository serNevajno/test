			...
			<div class="col-md-12">
				<div class="tabbable tabbable-custom">
					...
					<form action="" enctype="multipart/form-data" method="POST" name="form" id="myForm">
						<input name="func" type="hidden" value="banner">
									...
									<div class="form-group">
										<div class="checkbox-list">
											Активность:&nbsp;  <input type="checkbox" value="1" id="active" name="active">
											 | 
											Цвет текста на баннере:&nbsp;  <input type="color" id="color" name="color">
										</div>
									</div>	
									<div class="form-group">
						...
						<div class="form-actions">
							...
							<button type="button" class="btn red" onClick='location.href="/adminius/index.php?code=banner"'>Отмена</button>
						</div>
					</form>
				</div>
			</div>
									...
									<div class="form-group">
										<div class="checkbox-list">
											Активность:&nbsp;  <input type="checkbox" value="1" name="active" <?if ($item["active"] == 1){?>checked<?}?>>
											 | 
											Цвет текста на баннере:&nbsp;  <input type="color" id="color" name="color" value="<?=$item["color"]?>">
										</div>
									</div>
									<div class="form-group">
	...
	<?}else{?>
		...
		<div class="portlet box purple">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-cogs"></i>Управление баннерами
				</div>
				<div class="tools">
			...
			<div class="portlet-body">
				<div class="table-toolbar">
					<div class="btn-group">
						<button id="sample_editable_1_new" class="btn green" onClick='location.href="/adminius/index.php?code=banner&action=add"'>
					...
					<div class="table-scrollable">
						...
						<?$n=1+$start;
							foreach ($select_slider as $item){
								...
								<tr>
									<td><?=$n?></td>
									<td><a href="/adminius/index.php?code=banner&action=edit&id=<?=$item["id"]?>"><?=$item['title']?></a></td>
									<td><?=$item['descriptions']?></td>
									<td><span class="label label-sm label-<?=$class?>"><?=$act?><?$item["active"]?></span></td>
									<td><?=$item['priority']?></td>
									<td>
										<a class="btn btn-xs blue btn-editable" data-id="1" href="/adminius/index.php?code=banner&action=edit&id=<?=$item["id"]?>"><i class="fa fa-pencil"></i> Edit</a> 
										<?if($access["del"] == 1){?>
				...
				<?else:?>
					<h3> К сожалению но баннеров нет :(<h3>
				<?endif;?>
			</div>
		</div>
