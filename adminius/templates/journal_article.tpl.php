<?if($access["journal"] == 1){?>
	<?if (isset($_GET["action"])){?>
				...
				<div class="col-md-12">
					<div class="tabbable tabbable-custom">
						...
						<form role="form" action="" enctype="multipart/form-data" method="POST" name="form" id="myForm">
							<input name="func" type="hidden" value="journal_article">
							<input name="journal" type="hidden" value="<?=$_GET["journal"]?>">
							...
							<div class="form-actions">
								...
								<button type="button" class="btn red" onClick='location.href="/adminius/index.php?code=journal_article"'>Отмена</button>
							</div>
						</form>
					</div>
				</div>
		...
		<?}elseif($_GET["action"] == "edit"){?>
			<?$itemBlog = selectJournalArticleById($_GET["id"]);?>
			<!-- BEGIN PAGE CONTENT-->
			<div class="row">
				<div class="col-md-12">
					<div class="tabbable tabbable-custom">
							...
							<div class="tab-content">
								<div class="tab-pane active fontawesome-demo" id="tab_0">
									<div class="form-body">
													...
													<div class="form-group">
														<label>Выберите журнал</label>
														<select class="form-control" name="journal" id="journal">
															<?foreach (selectJournalAll() as $iJournal){?>
																<option value="journal" <?if ($item["journal"] == $iJournal['id']) echo "selected";?>><?=$iJournal['name']?></option>
															<?}?>
														</select>
													</div>
									</div>
									...
									<div class="form-group">
											<div class="thumbnail" style="width: 310px;">
														<?if(empty($itemBlog["img"])){?>
															<img src="http://www.placehold.it/310x170/EFEFEF/AAAAAA&amp;text=no+image" alt="">
														<?}else{?>
															<img src="/images/journal_articles/<?=$itemBlog["img"]?>" alt="">
														<?}?>
											</div>
											<div class="margin-top-10 fileupload fileupload-new" data-provides="fileupload">
												<div class="input-group input-group-fixed">
		...
		<div class="portlet box purple">
			...
			<div class="portlet-body">
				<div class="table-toolbar">
					<div class="btn-group">
						<button id="sample_editable_1_new" class="btn green" onClick='location.href="/adminius/index.php?code=journal_article&action=add&journal=<?=$_GET["journal"]?>"'>
						...
						<div class="dataTables_filter" id="sample_1_filter">
							<div class="col-md-6">
								<div class="input-group input-medium">
									<form action="/adminius/index.php" method="GET" style=" display: inline-table;"><input name="code" type="hidden" value="blog"/>
				...
				<?$select_blog = selectJournalArticle($page, $num, $search, $_GET["journal"]);
					...
					<div class="table-scrollable">
						...
						<?$n=1+$start;
							foreach ($select_blog as $item){
								...
								<tr>
									<td><?=$n?></td>
									<td><a href="/adminius/index.php?code=journal_article&action=edit&id=<?=$item["id"]?>"><?=$item['name']?></a></td>
									<td><span class="label label-sm label-<?=$class?>"><?=$act?><?$item["active"]?></span></td>
									<td><?=$item['date']?></td>
									<td>
										<a class="btn btn-xs blue btn-editable" data-id="1" href="/adminius/index.php?code=journal_article&action=edit&id=<?=$item["id"]?>"><i class="fa fa-pencil"></i> Edit</a> 
										<?if($access["del"] == 1){?>
				...
				<?else:?>
					<h3> К сожалению но статей нет :(<h3>
				<?endif;?>
			</div>
		</div>
