        ...
        <div class="portlet box blue">
            ...
            <div class="portlet-body">
                        ...
                        <div class="panel panel-default">
                            ...
                            <div id="collapse_1" class="panel-collapse collapse" style="height: 0px;">
                                <div class="portlet-body form">
                                    <form role="form" method="GET" action="/adminius/index.php?code=cashless">
                                        <input name="code" type="hidden" value="cashless">
                                        <div class="form-body">
                ...
                <table class="table table-striped table-bordered table-hover" id="sample_1">
                    ...
                    <? $select_order = selectCashless($page, $num, $_GET["from"], $_GET["to"], $_GET["status"]);
                    ...
                    foreach ($select_order as $item) {?>
                        <tr class="odd">
                            ...
                            <td class="">
                                 <?if($item["id_user"]>0){?>
                                    <a href="/adminius/index.php?code=users&action=detail&id=<?= $item["id_user"] ?>"><?= $item["name"] ?></a>
                                <?}else{?>
                                     <?= $item["name"] ?>
                                <?}?>
                            </td>
                            <td class=""><a href="/adminius/index.php?code=orders&action=edit&id=<?= $item["id"] ?>"><?= $item["id"] ?></a></td>
                            <td class=""><? if ($item['summ'] > 0) {
                                    echo $item['summ'];
                                } else {
                                    echo selectSummOrderById($item["id"]);
                                } ?> руб</td>
                            <td class="" id="btnConfirmCashless<?= $item["id"] ?>">
                                <?if(empty($item["date_cashless"]) AND empty($item["nomer"])){?>
                                    <?if($access["prepayment_confirm"] == 1){?>
                                         <a class="btn btn-xs blue btn-editable" data-toggle="modal"
                                        data-target="#confirmCashlessModal" onClick="confirmCashless('<?= $item["id"] ?>')"></i> Оплачено</a>
                                    <?}?>
                                <?}else{?>
                                    <?=$item["nomer"]." / ".$item["date_cashless"]?>
                                <?}?>
                            </td>
                        </tr>
                    <? $n++;} ?>
                ...
                <? include($_SERVER['DOCUMENT_ROOT'] . '/adminius/templates/modals/confirmCashless.tpl.php'); ?>
