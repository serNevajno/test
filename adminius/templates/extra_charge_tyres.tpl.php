						...
						<div class="tabbable tabbable-custom">
							...
							<form role="form" action="" enctype="multipart/form-data" method="POST" name="form" id="myForm">
										...
										<div class="form-body">
														...
														<div class="form-group">
															...
															<div class="radio-list">
																...
																<input type="radio" name="season" value="s" required> Летняя </label>
																<label class="radio-inline">
																<input type="radio" name="season" value="w" required> Зимняя </label>
																<label class="radio-inline">
																<input type="radio" name="season" value="n" required> Всесезоная </label>
															</div>
														</div>
								...
								<div class="form-actions">
									...
									<button type="button" class="btn red" onClick='location.href="/adminius/index.php?code=extra_charge_tyres"'>Отмена</button>
								</div>
							</form>
						</div>
					</div>
								...
								<input name="id" type="hidden" value="<?=$_GET["id"]?>">
										<div class="form-body">
														...
														<div class="form-group">
															...
															<div class="radio-list">
																<label class="radio-inline">
																<input type="radio" name="season" value="s" <?if($sExtraCharge["season"] == "s") echo "checked";?> required> Летняя </label>
																<label class="radio-inline">
																<input type="radio" name="season" value="w" <?if($sExtraCharge["season"] == "w") echo "checked";?> required> Зимняя </label>
																<label class="radio-inline">
																<input type="radio" name="season" value="n" <?if($sExtraCharge["season"] == "n") echo "checked";?> required> Всесезоная </label>
															</div>
														</div>
			...
			<div class="portlet box purple">
				...
				<div class="portlet-body">
					<div class="table-toolbar">
						<div class="btn-group">
							<button id="sample_editable_1_new" class="btn green" onClick='location.href="/adminius/index.php?code=extra_charge_tyres&action=add"'>
							...
							<form action="index.php" method="POST" name="basePercent">
								...
								<div class="form-group">
										<div class="input-group">
											...
											<input type="text" class="form-control input-small" name="percent" placeholder="%" value="<?=selectBasePercent("1");?>">
					...
					<?$sExtraCharge = selectExtraChargeTyres($page, $num, $_GET["brand"], $_GET["diameter"]);
							...
							<?$n=1+$start;
								foreach ($sExtraCharge as $item){
									if($item["season"] == "s"){
										...
										$season = "Зимняя";
									}elseif($item["season"] == "n"){
										$season = "Всесезоная";
									}
									?>
									<tr>
										<td><?=$n?></td>
										<td><a href="/adminius/index.php?code=extra_charge_tyres&action=edit&id=<?=$item["id"]?>"><?=$item['brand']?></a></td>
										<td><?if($item['diameter']>0){ echo $item['diameter']."R";}else{echo "Все";}?></td>
										<td><?=$season?></td>
										<td><?=$item['percent']?>%</td>
										<td>
											<a class="btn btn-xs blue btn-editable" data-id="1" href="/adminius/index.php?code=extra_charge_tyres&action=edit&id=<?=$item["id"]?>"><i class="fa fa-pencil"></i> Edit</a> 
											<?if($access["del"] == 1){?>
