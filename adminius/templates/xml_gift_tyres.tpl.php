<? if ($access["xml"] == 1) { ?>
  <? if (isset($_GET["action"])) { ?>
          ...
          <div class="tabbable tabbable-custom">
            ...
            <form role="form" action="" enctype="multipart/form-data" method="POST" name="form" id="myForm">
              ...
              <div class="form-actions">
                ...
                <button type="button" class="btn red" onClick='location.href="/adminius/index.php?code=xml_gift_tyres"'>
                  Отмена
                </button>
              </div>
            </form>
    ...
    <? } elseif ($_GET["action"] == "edit") { ?>
      ...
      $arrSeason = explode(",", $sExtraCharge["season"]);
      ?>
      <!-- BEGIN PAGE CONTENT-->
      <div class="row">
        <div class="col-md-12">
              ...
              <div class="form-body">
                <div class="form-group">
                  <div class="checkbox-list">
                    <b>Активность:</b>&nbsp; <input type="checkbox" value="1"
                                                    name="active" <? if ($sExtraCharge["active"]) echo "checked" ?>>
                  </div>
                </div>
                <div class="form-group">
                  <label>Название акции <font color="red">*</font></label>
                ...
                <div class="form-group">
                  ...
                  <select name="brand[]" class="form-control input-medium" required multiple>
                    <? foreach (selectTyresBrand() as $iBrand) { ?>
                      <option
                          value="<?= $iBrand["id"] ?>" <? if (in_array($iBrand["id"], $arrBrand)) echo 'selected="selected"'; ?>><?= $iBrand["name"] ?></option>
                    <? } ?>
                  </select>
                </div>
                <div class="form-group">
                  <label>Диаметр: <font color="red">*</font></label>
                  <select name="diameter[]" class="form-control input-medium" required multiple>
                    <option value="0" <? if (empty($sExtraCharge["diameter"])) echo 'selected="selected"'; ?>>Все
                    </option>
                    <? foreach (selectValueFilter('20') as $iDiameter) { ?>
                      <option
                          value="<?= $iDiameter["id"] ?>" <? if (in_array($iDiameter["id"], $arrDiameter)) echo 'selected="selected"'; ?>>
                        R<?= $iDiameter["value"] ?></option>
                    <? } ?>
                  </select>
                </div>
                ...
                <div class="form-group">
                  ...
                  <select name="season[]" class="form-control input-medium" required multiple>
                    ...
                    <option value="155" <? if (in_array('155', $arrSeason)) echo 'selected="selected"'; ?>>Летняя
                    </option>
                    <option value="156" <? if (in_array('156', $arrSeason)) echo 'selected="selected"'; ?>>Зимняя
                    </option>
                    <option value="157" <? if (in_array('157', $arrSeason)) echo 'selected="selected"'; ?>>Всесезоная
                    </option>
                  </select>
                </div>
                <div class="form-group">
                  ...
                  <div class="input-group">
                    ...
                    <input type="text" class="form-control" name="product" placeholder="Через запятую"
                           value="<?= $sExtraCharge["product"] ?>" required>
                  </div>
                </div>
              </div>
              <div class="form-actions">
  ...
  <? } else { ?>
    ...
    <div class="portlet box purple">
      ...
      <div class="portlet-body">
        <div class="table-toolbar">
          <div class="btn-group">
            <button id="sample_editable_1_new" class="btn green"
                    onClick='location.href="/adminius/index.php?code=xml_gift_tyres&action=add"'>
        ...
        <? $sExtraCharge = selectXMLGift($page, $num, "1");
          ...
          <div class="table-scrollable">
            <table class="table table-striped table-bordered table-hover">
              ...
              foreach ($sExtraCharge as $item) {
                ...
                <tr>
                  <td><span
                        class="ch label label-sm label-<?= $item["active"] == 1 ? "success" : "danger"; ?>"><?= $n ?></span>
                  </td>
                  <td><a
                        href="/adminius/index.php?code=xml_gift_tyres&action=edit&id=<?= $item["id"] ?>"><?= $item['name'] ?></a>
                  </td>
                  <td>
                    <a class="btn btn-xs blue btn-editable" data-id="1"
                       href="/adminius/index.php?code=xml_gift_tyres&action=edit&id=<?= $item["id"] ?>"><i
