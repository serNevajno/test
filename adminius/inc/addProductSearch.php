...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $id_prod = clearData($_POST['id_prod']);
  $date = date("Y-m-d H:i:s");

  $res = db2array("SELECT pr.name, pr.id, pr.code, pr.price, pr.price_clear, cat.id as cat_id, cat.img as cat_img, pr.rating, pr.sale, pr.attention, pr.availability, pr.logistic FROM categories as cat
    LEFT JOIN product as pr ON(pr.categories=cat.id) 
  WHERE pr.active='1' AND pr.id = '$id_prod'");

  $img = "/images/categories_cover/".$res['cat_img'];
  $parentCat = parentCategories($res['cat_id']);
  if($parentCat = 1){
    $parentCat = 'tyres';
  }elseif($parentCat = 2){
    $parentCat = 'disk';
  ...
  mysql_query("INSERT INTO basket (customer, quantity, date, product_id, price, name, img, categories, sale, day, price_clear) VALUES ('".session_id()."', '1', '$date', '$id_prod', '$res[price]', '$res[name]', '$img', '$parentCat', '$res[sale]', '$res[logistic]', '$res[price_clear]')") or die(mysql_error());
        ...
        $provider .='  <tr '.$style.'>
            <td >
              <label class="radio-inline">
                <input type="radio" onclick="changePosBascket('.$item['id'].', '.$priceProd['price'].', '.$priceProd['price_clear'].', '.$priceProd['logistic'].', '.$priceProd['id_provider'].', '.$n.')" name="provider['.$item[id].']" value="'.$priceProd[id_provider].'" '.$check.'> '.$priceProd[name].'
              </label>
            </td>
