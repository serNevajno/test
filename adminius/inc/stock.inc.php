...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	if (!isset($_POST["id"])) {
		...
		mysql_query("INSERT INTO blog (active, name, title, meta_d, meta_k, code, text, img, h1, description, date, id_user, date_active, cat) VALUES ('$active', '$name', '$title', '$meta_d', '$meta_k', '$code', '$text', '$foto', '$h1', '$description', '$date', '$id_user', '$date_active', '2')") or die(mysql_error());
		$id_element = mysql_insert_id();
		
		//addLogs(selectWhatUser(), 9, $id_element, "blog");

		header('Location: index.php?code=stock&action=edit&id='.$id_element);
		exit;
		...
		mysql_query("UPDATE blog SET active='$active', name='$name', title='$title', meta_d='$meta_d', meta_k='$meta_k', code='$code', description='$description', img='$foto', h1='$h1', text='$text', date_active='$date_active', cat='2' WHERE id='$id'") or die(mysql_error());
		

		//addLogs(selectWhatUser(), 10, $id, "blog");
		header("Location: index.php?code=stock");
		exit;
