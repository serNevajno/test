<?if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $cStart = $_POST["count"];
    $res = db2array("SELECT * FROM bufer_shs WHERE `type`='1' LIMIT $cStart, 50", 2);
   ...
   /* echo "<pre>";
    ...
    foreach ($res as $item){
            $id_provider = '9';
            $itemPr = selectProductByArtiсleForCSV($item["code"], 1);
            if ($itemPr) {
                $price = getPriceProvide("1", $itemPr["brand"], $itemPr["diameter"], $itemPr["season"], $item["price"], $id_provider); ///Розничная цена
                $availability = $item["quantity"]; ///Итого
                $logistic = 1;

                addProduct($id_provider, "1", $item["code"], $price, $item["price"], $logistic, $availability, "", "", "", "", "");
            }else{
