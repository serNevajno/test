...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	if (!isset($_POST['id'])){
		...
		// Заносим в базу
		mysql_query("INSERT INTO slider (title, descriptions, img, url, active, priority, color, type) VALUES ('$title', '$discriptions', '$foto_light_name', '$url', '$active', '$priority', '$color', '$type')") or die(mysql_error());
		$id_element = mysql_insert_id();
		addLogs(selectWhatUser(), 6, $id_element, "slider");

		header('Location: index.php?code=slider&action=edit&id='.$id_element);
		exit;
		...
		// Заносим в базу
		mysql_query("UPDATE slider SET title='$title', descriptions='$discriptions', img='$foto_light_name', url='$url', active='$active', priority='$priority', color='$color', type='$type' WHERE id='$id'") or die(mysql_error());
		
		addLogs(selectWhatUser(), 7, $id, "slider");
		
		if(isset($_POST['submit'])):
			header('Location: /adminius/index.php?code=slider');
		elseif(isset($_POST['submit1'])):
			header('Location: /adminius/index.php?code=slider&action=edit&id='.$id);
		endif;
		exit;
		...
		header('Location: index.php?code=slider');
		exit;
	}
}?>