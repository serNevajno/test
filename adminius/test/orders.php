...
include($_SERVER['DOCUMENT_ROOT'] . '/adminius/test/header.php');?>
  <div class="clearfix">
  </div>
  <!-- BEGIN CONTAINER -->
              ...
              <div class="col-md-2">
                ...
                <? if ($item['id_status'] != "3" AND $item['id_status'] != "1") { ?>
                  <a class="btn btn-xs blue btn-editable" data-id="1"
                     onclick="if (confirm('Отправить SMS?')) {$('#sms').val(1);refform.submit();} else {$('#sms').val(0);refform.submit();}" style="height: 25px; margin-left: 5px; padding: 4px;"><i class="fa fa-refresh"></i>
                  </a>
                <? } ?>
                </div>
              </div>
                    ...
                    <td class="">
                      ...
                      <? } else { ?>
                        ...
                        <a class="btn btn-xs yellow btn-removable" data-toggle="modal" data-target="#modalStorage"
                           onClick="changeStorage('<?= $item_order['id'] ?>', '<?= $item["region_id"] ?>')">Оприходовать
                                                                                                            товар</a>
                      <? } ?>
                    </td>
                    <td class="">
